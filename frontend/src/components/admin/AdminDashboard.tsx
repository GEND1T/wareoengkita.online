...
export const AdminDashboard: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[2500] bg-[#F9F8F6] flex overflow-hidden font-sans selection:bg-[#77a160] selection:text-white animate-fade-in">
      ...
      {/* ---------------------------------------------------- */}
      <aside
        className={`bg-[#F9F8F6] border-r border-gray-200/80 flex flex-col justify-between shrink-0 shadow-sm z-20 transition-all duration-300 ${
      ...
      >
              ...
              {superadminMenuItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    title={item.label}
                    className={`relative w-full flex items-center rounded-2xl text-xs font-extrabold transition-all duration-200 ${
                      isSidebarCollapsed
                        ? 'justify-center p-3'
                      ...
                      className={`font-black ${
                        isSidebarCollapsed
                          ...
                          : `px-2 py-0.5 rounded-full text-[10px] ${
                              isActive ? 'bg-white text-[#063104]' : 'bg-red-500 text-white animate-pulse'
                            }`
                      }`}
                    >
                      {item.badge}
        ...
        <div className="pt-3 border-t border-gray-200/80 w-full shrink-0">
          <button
            ...
            className={`w-full bg-white hover:bg-emerald-50 text-[#063104] hover:text-[#063104] border border-emerald-200/80 font-extrabold rounded-2xl text-xs transition-all flex items-center justify-center gap-2 shadow-xs ${
              isSidebarCollapsed ? 'p-3' : 'py-2.5 px-3.5'
            }`}
          >
        ...
        <header className="bg-white border-b border-gray-200/80 px-6 py-3.5 flex items-center justify-between gap-4 shrink-0 shadow-xs z-10">
            ...
            >
              <PanelLeftClose
                className={`w-5 h-5 transition-transform duration-300 ${
                  isSidebarCollapsed ? 'rotate-180 text-[#063104] hover:text-white' : ''
                }`}
              />
            </button>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex items-center gap-2.5 pl-2 border-l border-gray-200">
              <div
                className={`w-8.5 h-8.5 rounded-full font-black text-xs flex items-center justify-center text-white shadow-xs ${
                  profile.role === 'superadmin' ? 'bg-purple-900' : 'bg-[#063104]'
                }`}
              >
              ...
              <div className="hidden sm:block text-left min-w-0">
                ...
                <span
                  className={`text-[10px] font-extrabold block leading-tight truncate max-w-[140px] ${
                    profile.role === 'superadmin' ? 'text-purple-700' : 'text-emerald-700'
                  }`}
                >
