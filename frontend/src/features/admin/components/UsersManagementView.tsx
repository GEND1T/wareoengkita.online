    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-xs flex items-center justify-between">
          <div>
            <span className="text-xs text-gray-500 font-bold block">Total Seluruh User</span>
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-xs flex items-center justify-between">
          <div>
            <span className="text-xs text-gray-500 font-bold block">Pelanggan (Customer)</span>
            <span className="text-2xl font-black text-[#063104]">{totalCustomers}</span>
          </div>
          <div className="w-10 h-10 rounded-xl bg-emerald-100 text-[#063104] flex items-center justify-center">
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-xs flex items-center justify-between">
          <div>
            <span className="text-xs text-gray-500 font-bold block">Admin Store (Cabang)</span>
            <span className="text-2xl font-black text-blue-800">{totalStoreAdmins}</span>
          </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-xs flex items-center justify-between">
          <div>
            <span className="text-xs text-gray-500 font-bold block">Superadmin</span>
            <span className="text-2xl font-black text-purple-800">{totalSuperadmins}</span>
          </div>
      ...
      <div className="bg-white rounded-3xl p-4 border border-gray-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        {/* Role Tab Filter Pills */}
        <div className="flex items-center gap-1.5 overflow-x-auto w-full md:w-auto">
          {[
            { id: 'all', label: 'Semua User' },
            { id: 'customer', label: 'Customer' },
            { id: 'admin_store', label: 'Admin Store' },
            ...
            <button
              ...
              className={`px-3.5 py-2 rounded-xl text-xs font-extrabold transition-all whitespace-nowrap ${roleFilter === tab.id
                ...
                filteredUsers.map((u) => (
                  <tr key={u.id} className="hover:bg-emerald-50/30 transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-extrabold text-gray-900">
                      {u.role === 'admin_store'
                        ? formatCurrency(u.totalOrdersOrSales)
                        : `${u.totalOrdersOrSales} Pesanan`}
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-center">
                      ...
                      >
                        <span
                          className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-md ring-0 transition duration-200 ease-in-out ${u.status === 'active' ? 'translate-x-5' : 'translate-x-0'
                            }`}
                        />
                      </button>
