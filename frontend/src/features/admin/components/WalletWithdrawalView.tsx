...
export const WalletWithdrawalView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-[#063104] to-[#0d530b] rounded-2xl md:rounded-3xl p-4 md:p-6 text-white shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Wallet className="w-5 md:w-6 h-5 md:h-6 text-emerald-400" />
            <h2 className="text-base md:text-xl font-extrabold">Dompet & Pencairan Dana</h2>
          </div>
          <p className="text-emerald-200/90 text-[10px] md:text-xs font-medium max-w-xl hidden md:block">
        ...
        <button
          ...
          className="bg-white/10 hover:bg-white/20 text-white border border-white/20 px-3 py-1.5 md:px-3.5 md:py-2 rounded-xl text-xs font-bold flex items-center gap-1.5 self-start md:self-auto transition-all cursor-pointer active:scale-95"
        >
        ...
        <div className="md:col-span-6 bg-white rounded-3xl p-6 border border-gray-100 shadow-sm flex flex-col justify-between space-y-4">
          <div>
            ...
            <div className="mt-2 flex items-baseline gap-2">
              <span className="text-2xl md:text-3xl lg:text-4xl font-black text-gray-900 leading-none">
      ...
      <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm space-y-4">
        <div className="flex items-center gap-2 md:gap-3 border-b border-gray-100 pb-3 overflow-x-auto no-scrollbar">
          <button
            type="button"
            onClick={() => setActiveTab('mutations')}
            className={`px-3 md:px-4 py-1.5 md:py-2 rounded-xl text-[11px] md:text-xs font-extrabold transition-all cursor-pointer whitespace-nowrap ${
              activeTab === 'mutations'
                    ...
                    <div key={m.id} className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-1.5">
                          {m.type === 'CREDIT' && (
                            <span className="bg-emerald-50 text-emerald-800 border border-emerald-200 text-[9px] font-black px-2 py-0.5 rounded-full inline-flex items-center gap-0.5">
                              <ArrowDownLeft className="w-2.5 h-2.5" /> MASUK
                            </span>
                          )}
                          {m.type === 'DEBIT' && (
                            <span className="bg-amber-50 text-amber-900 border border-amber-200 text-[9px] font-black px-2 py-0.5 rounded-full inline-flex items-center gap-0.5">
                              <ArrowUpRight className="w-2.5 h-2.5" /> KELUAR
                            </span>
                          )}
                          {m.type === 'REFUND' && (
                            <span className="bg-blue-50 text-blue-900 border border-blue-200 text-[9px] font-black px-2 py-0.5 rounded-full inline-flex items-center gap-0.5">
                              <RefreshCw className="w-2.5 h-2.5" /> REFUND
                            </span>
                          )}
                        </div>
                        <span className={`text-xs font-extrabold ${m.type === 'CREDIT' || m.type === 'REFUND' ? 'text-emerald-700' : 'text-red-600'}`}>
                          {m.type === 'CREDIT' || m.type === 'REFUND' ? '+' : '-'} Rp {m.amount.toLocaleString('id-ID')}
                        </span>
                      </div>
                      <p className="text-[10px] font-semibold text-gray-700 truncate">{m.description}</p>
                      <div className="flex items-center justify-between mt-1 text-[9px] text-gray-400">
                        <span>{new Date(m.createdAt).toLocaleString('id-ID', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>
                        <span>Saldo: <strong className="text-gray-600">Rp {m.balanceAfter.toLocaleString('id-ID')}</strong></span>
                      </div>
                    </div>
                  ))}
                </div>
                      ...
                      {mutations.map((m) => (
                        <tr key={m.id} className="hover:bg-gray-50/80 transition-colors">
                          ...
                          <td className="py-3.5 px-3">
                            {m.type === 'CREDIT' && (<span className="bg-emerald-50 text-emerald-800 border border-emerald-200 text-[10px] font-black px-2.5 py-1 rounded-full inline-flex items-center gap-1"><ArrowDownLeft className="w-3 h-3" /> MASUK</span>)}
                            {m.type === 'DEBIT' && (<span className="bg-amber-50 text-amber-900 border border-amber-200 text-[10px] font-black px-2.5 py-1 rounded-full inline-flex items-center gap-1"><ArrowUpRight className="w-3 h-3" /> PENARIKAN</span>)}
                            {m.type === 'REFUND' && (<span className="bg-blue-50 text-blue-900 border border-blue-200 text-[10px] font-black px-2.5 py-1 rounded-full inline-flex items-center gap-1"><RefreshCw className="w-3 h-3" /> REFUND</span>)}
                          </td>
                          <td className="py-3.5 px-3 font-semibold text-gray-800">{m.description}</td>
                          <td className={`py-3.5 px-3 text-right font-extrabold ${m.type === 'CREDIT' || m.type === 'REFUND' ? 'text-emerald-700' : 'text-red-600'}`}>
                            {m.type === 'CREDIT' || m.type === 'REFUND' ? '+' : '-'} Rp {m.amount.toLocaleString('id-ID')}
                          </td>
                ...
                <div className="md:hidden space-y-2">
                  {withdrawals.map((w) => (
                    <div key={w.id} className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="font-mono font-extrabold text-[10px] text-[#063104]">{w.withdrawalNo}</span>
                        {w.status === 'PENDING' && (
                          <span className="bg-amber-50 text-amber-900 border border-amber-200 text-[9px] font-extrabold px-2 py-0.5 rounded-full inline-flex items-center gap-0.5">
                            <Clock className="w-2.5 h-2.5" /> PENDING
                          </span>
                        )}
                        {w.status === 'SUCCESS' && (
                          <span className="bg-emerald-50 text-emerald-800 border border-emerald-200 text-[9px] font-extrabold px-2 py-0.5 rounded-full inline-flex items-center gap-0.5">
                            <CheckCircle2 className="w-2.5 h-2.5" /> BERHASIL
                          </span>
                        )}
                        {w.status === 'FAILED' && (
                          <span className="bg-red-50 text-red-800 border border-red-200 text-[9px] font-extrabold px-2 py-0.5 rounded-full inline-flex items-center gap-0.5">
                            <XCircle className="w-2.5 h-2.5" /> GAGAL
                          </span>
                        )}
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] text-gray-500">{w.bankCode} ({w.accountNumber})</span>
                        <span className="text-xs font-extrabold text-[#063104]">Rp {w.netAmount.toLocaleString('id-ID')}</span>
                      </div>
                      <div className="flex items-center justify-between mt-1 text-[9px] text-gray-400">
                        <span>{new Date(w.createdAt).toLocaleString('id-ID', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>
                        <span className="text-red-500">Fee: -Rp {w.disbursementFee.toLocaleString('id-ID')}</span>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    <tbody className="divide-y divide-gray-100 text-xs">
                      {withdrawals.map((w) => (
                        <tr key={w.id} className="hover:bg-gray-50/80 transition-colors">
                          <td className="py-3.5 px-3 font-mono font-extrabold text-[#063104]">{w.withdrawalNo}</td>
                          <td className="py-3.5 px-3 text-gray-500 font-medium">
                          ...
                          <td className="py-3.5 px-3 font-bold text-gray-800">
                            ...
                            <span className="block text-[10px] text-gray-400 font-normal">a.n. {w.accountHolder}</span>
                          </td>
                          <td className="py-3.5 px-3 text-right font-bold text-gray-800">Rp {w.amount.toLocaleString('id-ID')}</td>
                          <td className="py-3.5 px-3 text-right font-semibold text-red-600">- Rp {w.disbursementFee.toLocaleString('id-ID')}</td>
                          <td className="py-3.5 px-3 text-right font-extrabold text-[#063104]">Rp {w.netAmount.toLocaleString('id-ID')}</td>
                          <td className="py-3.5 px-3 text-center">
                            {w.status === 'PENDING' && (<span className="bg-amber-50 text-amber-900 border border-amber-200 text-[10px] font-extrabold px-2.5 py-1 rounded-full inline-flex items-center gap-1"><Clock className="w-3 h-3 text-amber-600" /> PENDING</span>)}
                            {w.status === 'SUCCESS' && (<span className="bg-emerald-50 text-emerald-800 border border-emerald-200 text-[10px] font-extrabold px-2.5 py-1 rounded-full inline-flex items-center gap-1"><CheckCircle2 className="w-3 h-3 text-emerald-600" /> BERHASIL</span>)}
                            {w.status === 'FAILED' && (<span className="bg-red-50 text-red-800 border border-red-200 text-[10px] font-extrabold px-2.5 py-1 rounded-full inline-flex items-center gap-1" title={w.failureReason || 'Gagal'}><XCircle className="w-3 h-3 text-red-600" /> GAGAL</span>)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
